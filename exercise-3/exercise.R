...
USPE.data <- data.frame(USPersonalExpenditure)
...
USPE.data$category <- rownames(USPE.data)

# How much money was spent on personal care in 1940?
USPE.data["Personal Care", "X1940"]

# How much money was spent on Food and Tobacco in 1960
USPE.data["Food and Tobacco", "X1960"]

# What was the highest expenditure category in 1960?
USPE.data$category[USPE.data$X1960 == max(USPE.data$X1960)]
...
FindHighest <- function(year) {
  return(USPE.data$category[USPE.data[year] == max(USPE.data[year])])
}
# Using your function, determine the highest spending category of each year
years <- colnames(USPE.data)
